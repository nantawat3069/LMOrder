...
function ShopHistory() {
    ...
    useEffect(() => {
        ...
        if(u.shop_id) fetchHistory(u.shop_id);
             ...
             axios.get(`http://192.168.1.37/LMOrder/api/shop.php?action=get_shop_data&owner_id=${u.id}`)
                .then(res => fetchHistory(res.data.shop.id));
        }
    }, []);

    const fetchHistory = async (sid) => {
        const res = await axios.get(`http://192.168.1.37/LMOrder/api/order.php?action=get_shop_orders&shop_id=${sid}&type=history`);
